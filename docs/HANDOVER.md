...
## 2. 环境与依赖
- 数据库 DSN 固定在 `database/db.go`：`admin_sql:admin_sql@tcp(127.0.0.1:3306)/goland_demo?charset=utf8mb4&parseTime=True&loc=Local`。
- 启动时 `InitDB` 会 AutoMigrate `models.User`（以及 `InitTables` 中的 `Role` 占位）。
...
## 4. 启动与联调
1. 确保 MySQL 运行且存在数据库 `goland_demo`，可用账户 `admin_sql/admin_sql`。
2. 运行服务：`go run main.go`（默认端口 9090）。
3. 接口验证示例：
...
6. **配置化密钥/数据库**：将 DSN、JWT 密钥等提取到配置或环境变量，避免硬编码。
